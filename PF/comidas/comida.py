...
def crear (comida,precio):
    try:
        fecha=datetime.datetime.now()
        cursor.execute("INSERT INTO comida (nombre, precio) VALUES (%s, %s)",
                       (comida, precio))
        conexion.commit()
        return True
    except:
...
def buscar(buscar):
    try:
        cursor.execute("Select * from  comida where nombre=%s",(buscar,))
        return cursor.fetchall()
    except:
        return False 
...
def buscar2(buscar):
    try:
        cursor.execute("Select * from  comida where id=%s",(buscar,))
        return cursor.fetchall()
    except:
        return False
...
def cambiar(id):
    try:
        ...
        cursor.execute("SELECT * FROM comida WHERE id=%s", (id,))
        fila = cursor.fetchone()
        
        if fila:
            funciones.borrarPantalla()
            print("\n\t\U0001F501 Comida a modificar del menu: \n")
            ...
            if seguro == "si":
                ...
                cursor.execute(
                    "UPDATE comida SET nombre=%s, precio=%s WHERE id=%s",
                    (nombre, precio, id )
                )
                conexion.commit()
                return True
            ...
            print("\u26A0 No existe esta comida en el menu. \u26A0")
            return False
...
def borrar (id,):
    try:
        ...
        if registros:
            funciones.borrarPantalla()
            print("\n\t\U0001F4DB Comida a eliminar del menu: \n")
            print(f"{"ID":<10}{"Nombre":<30}{"Precio":<15}")
            print(f"-"*80)
            for fila in registros:
                print(f"{fila[0]:<10}{fila[1]:<30}{fila[2]:<15}")
